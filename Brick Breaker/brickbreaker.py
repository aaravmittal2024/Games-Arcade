...
class Brick:
    ...
    def collide(self, ball):
        if not (self.x <= ball.x <= self.x + self.width):
            return False
...
def draw(win, paddle, ball, bricks, lives):
    ...
    for brick in bricks:
        brick.draw(win)
    lives_text = LIVES_FONT.render(f"Lives: {lives}", 1, (255, 255, 255))
    ...
    for row in range(rows):
        for col in range(cols):
            brick = Brick(col * brick_width + gap * col, row * brick_height + gap * row,
                          brick_width, brick_height, 1, [(255, 0, 0), (255, 0, 0)])
            bricks.append(brick)
    return bricks

def main():
    clock = pygame.time.Clock()
    paddle_x, paddle_y = WIDTH // 2 - PADDLE_WIDTH // 2, HEIGHT - PADDLE_HEIGHT - 5
    paddle = Paddle(paddle_x, paddle_y, PADDLE_WIDTH, PADDLE_HEIGHT, (255, 255, 255))
    ball = Ball(WIDTH // 2, paddle_y - BALL_RADIUS, BALL_RADIUS, (255, 255, 255))
    ...
    def display_text(text):
        text_render = LIVES_FONT.render(text, 1, (255, 255, 255))
